def call (body) {
  ...
  container('alpine-harbor-scanner') {
    sh '''
      ...
      IMAGE_TAG="$(cat /artifacts/${PROJECT}/${PROJECT_MODE}/stg.artifact)"
      GIT_URL=${GIT_URL}
      GIT_EMAIL=${GIT_EMAIL}
      GIT_USER=${GIT_USER}
      ...
      yq -i ".deployments.[env(TARGET_SERVICE)].image.tag = env(TARGET_TAG)" values-stg.yaml

      echo "   ✓ Updated values-stg.yaml for ${PROJECT_MODE} with tag: ${IMAGE_TAG}"
